...
function processOptionArrays(allOptions) {
	processOptionArraysHelper(allOptions, []);
}

//
//	Eliminate the need for an explicit index
//

function processOptionArraysHelper(allOptions, row) {
	if (allOptions.length > 0) {
		let [first, ...rest] = allOptions;
		first.forEach(value => {
			processOptionArraysHelper(rest, row.concat(value));
		});
	} else {
		processValues(row);
	}
